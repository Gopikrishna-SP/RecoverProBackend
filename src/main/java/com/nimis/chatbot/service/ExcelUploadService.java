    ...
    public class ExcelUploadService {
        ...
        // ================= GET SINGLE =================
        public LoanFullResponse getLoanByLoanNumber(String loanNumber) {

            Allocation allocation = allocationRepository
                    .findByLoanNumber(loanNumber)
                    .orElseThrow(() -> new RuntimeException("Loan not found"));

            LoanFullResponse response = new LoanFullResponse();

            response.setAllocation(mapAllocation(allocation));
            customerRepository.findByAllocation(allocation)
                    .ifPresent(c -> response.setCustomer(mapCustomer(c)));
            phoneRepository.findByAllocation(allocation)
                    .ifPresent(p -> response.setPhone(mapPhone(p)));
            addressRepository.findByAllocation(allocation)
                    .ifPresent(a -> response.setAddress(mapAddress(a)));
            bankAccountRepository.findByAllocation(allocation)
                    .ifPresent(b -> response.setBankAccount(mapBankAccount(b)));
            agencyRepository.findByAllocation(allocation)
                    .ifPresent(a -> response.setAgency(mapAgency(a)));
            legalCaseRepository.findByAllocation(allocation)
                    .ifPresent(l -> response.setLegalCase(mapLegalCase(l)));

            return response;
        }

        // ================= GET ALL =================
        public List<LoanFullResponse> getAllLoans() {

            List<Allocation> allocations = allocationRepository.findAll();
            List<LoanFullResponse> result = new ArrayList<>();

            for (Allocation allocation : allocations) {
                result.add(getLoanByLoanNumber(allocation.getLoanNumber()));
            }
            return result;
        }
        ...
        private AllocationResponse mapAllocation(Allocation a) {
            ...
            r.setLoanNumber(a.getLoanNumber());
            r.setSegment(a.getSegment());
            r.setProduct(a.getProduct());
            r.setZone(a.getZone());
            r.setState(a.getState());
            r.setBranch(a.getBranch());
            r.setCustomerName(a.getCustomerName());
            r.setEmi(a.getEmi());
            r.setPosAmt(a.getPosAmt());
            return r;
        }

        private CustomerResponse mapCustomer(Customer c) {
            CustomerResponse r = new CustomerResponse();
            r.setMainApplicantName(c.getMainApplicantName());
            r.setMainApplicantMobile(c.getMainApplicantMobile());
            r.setPanMainApp(c.getPanMainApp());
            r.setDobMainApp(c.getDobMainApp());
            r.setCoApplicantName(c.getCoApplicantName());
            r.setCoApplicantMobile(c.getCoApplicantMobile());
            r.setPanCoApp(c.getPanCoApp());
            r.setDobCoApp(c.getDobCoApp());
            return r;
        }

        private PhoneResponse mapPhone(Phone p) {
            PhoneResponse r = new PhoneResponse();
            r.setPhone1(p.getPhone_1());
            r.setPhone2(p.getPhone_2());
            r.setPhone3(p.getPhone_3());
            return r;
        }

        private AddressResponse mapAddress(Address a) {
            AddressResponse r = new AddressResponse();
            r.setAddress1(a.getAddress_1());
            r.setAddress2(a.getAddress_2());
            r.setAddress3(a.getAddress_3());
            r.setMainPinCode(a.getMainPinCode());
            return r;
        }

        private BankAccountResponse mapBankAccount(BankAccount b) {
            BankAccountResponse r = new BankAccountResponse();
            r.setAccountHolder1(b.getAccountHolder1());
            r.setBankName1(b.getBankName1());
            r.setAccountNumber1(b.getAccountNumber1());
            r.setIfscCode1(b.getIfscCode1());
            return r;
        }

        private AgencyResponse mapAgency(Agency a) {
            AgencyResponse r = new AgencyResponse();
            r.setAgencyCode(a.getAgencyCode());
            r.setAgencyName(a.getAgencyName());
            r.setManagerName(a.getManagerName());
            return r;
        }

        private LegalCaseResponse mapLegalCase(LegalCase l) {
            LegalCaseResponse r = new LegalCaseResponse();
            r.setCaseType(l.getCaseType());
            r.setProcessStage(l.getProcessStage());
            r.setNoticeDate(l.getNoticeDate());
            r.setCourtForum(l.getCourtForum());
