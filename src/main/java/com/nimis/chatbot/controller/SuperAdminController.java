...
public class SuperAdminController {
    ...
    public ResponseEntity<BankResponseDTO> createBank(@Valid @RequestBody CreateBankRequest req) {
        return ResponseEntity.ok(adminService.createBank(req));
    }

    @PostMapping("/bank-admins")
    @PreAuthorize("hasRole('SUPER_ADMIN')")
    public ResponseEntity<UserResponseDTO> createBankAdmin(@Valid @RequestBody CreateBankAdminRequest req) {
